---
title: "Absolute Winding Number by fx Probe Day d8 "
...
# Day d8 Probe by fx Vol


``` {r fxVOLbbDayd8Probe, echo=FALSE}

plotfxVOL <- ggplot(geno_combined_VOL, aes(x = fx, y = Probe_d8,
  ...
  #geom_text(x=3,y=30,label=lm_eqn(geno_combined_VOL, 'Probe_d8','fx'),color='red',parse=T) 
...
plotfxVOLSex <- ggplot(geno_combined_VOL, aes(fx, Probe_d8, 
...
#####
lm <- lm(Probe_d8 ~ fx*Genotype*Sex, geno_combined_VOL)
#summary(lm)
lm_f <- lm(Probe_d8 ~ fx*Genotype, combo_f_VOL)
#summary(lm_f)
lm_m <- lm(Probe_d8 ~ fx*Genotype, combo_m_VOL)
#summary(lm_m)

#lm$coefficients

sink(paste(outpath, "Winding_Probed8_by_fx_VOL.txt")) 
'fx: Probe d8 AWN VOL'
lm <- lm(Probe_d8 ~ fx*Genotype*Sex, geno_combined_VOL)
summary(lm)
lm$coefficients
...
'slopes'
print('VOL slopes')
em1<-emtrends(lm, pairwise ~ Genotype, var = "fx")
em2<-emtrends(lm, pairwise ~ Genotype|Sex, var = "fx")
em3<-emtrends(lm, pairwise ~ Sex|Genotype, var = "fx")
...
# Day d8 Probe By fx FA

``` {r fxFAybDayd8Probe}

plotfxFA <- ggplot(geno_combined_FA, aes(x = fx, y = Probe_d8,
                          color = Genotype,  fill = Genotype)) + 
  scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Sex)) +
...
plotfxFASex <- ggplot(geno_combined_FA, aes(fx, Probe_d8, 
                                   color = Genotype, fill = Genotype))+                   scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Genotype)) + 
  facet_grid(. ~ Sex) + 
...
#####
lm <- lm(Probe_d8 ~ fx*Genotype*Sex, geno_combined_FA)
#summary(lm)
lm_f <- lm(Probe_d8 ~ fx*Genotype, combo_f_FA)
#summary(lm_f)
lm_m <- lm(Probe_d8 ~ fx*Genotype, combo_m_FA)
#summary(lm_m)

summary(lm)
lm$coefficients

sink(paste(outpath, "Winding_Probed8_by_fx_FA.txt")) 
'fx: Probe d8 AWN FA'
lm <- lm(Probe_d8 ~ fx*Genotype*Sex, geno_combined_FA)
'omnibus test'
joint_tests(ref_grid(lm)) 
...
# Day d8 Probe By fx DEG

``` {r fxDEGybDayd8Probe}

plotfxDEG <- ggplot(geno_combined_DEG, aes(x = fx, y = Probe_d8,
                          color = Genotype,  fill = Genotype)) + 
        scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
        scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
        geom_point(aes(shape = Sex)) +
...
plotfxDEGSex <- ggplot(geno_combined_DEG, aes(fx, Probe_d8, 
                                   color = Genotype, fill = Genotype))+                   scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Genotype)) + 
  facet_grid(. ~ Sex) + 
...
#####
lm <- lm(Probe_d8 ~ fx*Genotype*Sex, geno_combined_DEG)
summary(lm)
lm$coefficients
lm_f <- lm(Probe_d8 ~ fx*Genotype, combo_f_DEG)
#summary(lm_f)
lm_m <- lm(Probe_d8 ~ fx*Genotype, combo_m_DEG)
#summary(lm_m)

sink(paste(outpath, "Winding_Probed8_by_fx_DEG.txt")) 
'fx: Probe d8 AWN DEG'
c
'omnibus test'
joint_tests(ref_grid(lm)) 
...
# Day d8 Probe By fx CLUS

``` {r fxCLUSybDayd8Probe}




plotfxCLUS <- ggplot(geno_combined_CLUS, aes(x = fx, y = Probe_d8,
                          color = Genotype,  fill = Genotype)) + 
  scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Sex)) +
...
plotfxCLUSSex <- ggplot(geno_combined_CLUS, aes(fx, Probe_d8, 
                                   color = Genotype, fill = Genotype))+                   scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Genotype)) + 
  facet_grid(. ~ Sex) + 
...
lm <- lm(Probe_d8 ~ fx*Genotype*Sex, geno_combined_CLUS)
#summary(lm)
lm_f <- lm(Probe_d8 ~ fx*Genotype, combo_f_CLUS)
#summary(lm_f)
lm_m <- lm(Probe_d8 ~ fx*Genotype, combo_m_CLUS)
#summary(lm_m)

summary(lm)
lm$coefficients

sink(paste(outpath, "Winding_Probed8_by_fx_CLUS.txt")) 
'fx: Probe d8 AWN CLUSTERING'
lm <- lm(Probe_d8 ~ fx*Genotype*Sex, geno_combined_CLUS)
'omnibus test'
joint_tests(ref_grid(lm)) 
joint_tests(ref_grid(lm), by = "Sex", adjust = "sidak") 
...
ggsave(paste(outpath,'Probed8_Winding_by_fx.pdf',sep=''), plot = last_plot(), device='pdf', scale=1, width=10, height=20, unit=c("in"), dpi=200)

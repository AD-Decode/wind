---
title: "Absolute Winding Number by fi Probe Day d8 "
...
# Day d8 Probe by fi Vol


``` {r fiVOLbbDayd8Probe, echo=FALSE}

plotfiVOL <- ggplot(geno_combined_VOL, aes(x = fi, y = Probe_d8,
  ...
  #geom_text(x=3,y=30,label=lm_eqn(geno_combined_VOL, 'Probe_d8','fi'),color='red',parse=T) 
...
plotfiVOLSex <- ggplot(geno_combined_VOL, aes(fi, Probe_d8, 
...
sink(paste(outpath, "Winding_Probed8_by_fi_VOL.txt")) 
'fi: Probe d8 AWN VOL'
lm <- lm(Probe_d8 ~ fi*Genotype*Sex, geno_combined_VOL)
summary(lm)
lm$coefficients
...
'slopes'
print('VOL slopes')
em1<-emtrends(lm, pairwise ~ Genotype, var = "fi")
em2<-emtrends(lm, pairwise ~ Genotype|Sex, var = "fi")
em3<-emtrends(lm, pairwise ~ Sex|Genotype, var = "fi")
...
# Day d8 Probe By fi FA

``` {r fiFAybDayd8Probe}

plotfiFA <- ggplot(geno_combined_FA, aes(x = fi, y = Probe_d8,
                          color = Genotype,  fill = Genotype)) + 
  scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Sex)) +
...
plotfiFASex <- ggplot(geno_combined_FA, aes(fi, Probe_d8, 
                                   color = Genotype, fill = Genotype))+                   scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Genotype)) + 
  facet_grid(. ~ Sex) + 
...
sink(paste(outpath, "Winding_Probed8_by_fi_FA.txt")) 
'fi: Probe d8 AWN FA'
lm <- lm(Probe_d8 ~ fi*Genotype*Sex, geno_combined_FA)
'omnibus test'
joint_tests(ref_grid(lm)) 
...
# Day d8 Probe By fi DEG

``` {r fiDEGybDayd8Probe}

plotfiDEG <- ggplot(geno_combined_DEG, aes(x = fi, y = Probe_d8,
                          color = Genotype,  fill = Genotype)) + 
        scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
        scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
        geom_point(aes(shape = Sex)) +
...
plotfiDEGSex <- ggplot(geno_combined_DEG, aes(fi, Probe_d8, 
                                   color = Genotype, fill = Genotype))+                   scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Genotype)) + 
  facet_grid(. ~ Sex) + 
...
sink(paste(outpath, "Winding_Probed8_by_fi_DEG.txt")) 
'fi: Probe d8 AWN DEG'
c
'omnibus test'
joint_tests(ref_grid(lm)) 
...
# Day d8 Probe By fi CLUS

``` {r fiCLUSybDayd8Probe}




plotfiCLUS <- ggplot(geno_combined_CLUS, aes(x = fi, y = Probe_d8,
                          color = Genotype,  fill = Genotype)) + 
  scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Sex)) +
...
plotfiCLUSSex <- ggplot(geno_combined_CLUS, aes(fi, Probe_d8, 
                                   color = Genotype, fill = Genotype))+                   scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Genotype)) + 
  facet_grid(. ~ Sex) + 
...
sink(paste(outpath, "Winding_Probed8_by_fi_CLUS.txt")) 
'fi: Probe d8 AWN CLUSTERING'
lm <- lm(Probe_d8 ~ fi*Genotype*Sex, geno_combined_CLUS)
'omnibus test'
joint_tests(ref_grid(lm)) 
joint_tests(ref_grid(lm), by = "Sex", adjust = "sidak") 
...
ggsave(paste(outpath,'Probed8_Winding_by_fi.pdf',sep=''), plot = last_plot(), device='pdf', scale=1, width=10, height=20, unit=c("in"), dpi=200)

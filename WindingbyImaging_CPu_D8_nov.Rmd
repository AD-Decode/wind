---
title: "Absolute Winding Number by CPu Probe Day d8 "
...
# Day d8 Probe by CPu Vol


``` {r CPuVOLbbDayd8Probe, echo=FALSE}

plotCPuVOL <- ggplot(geno_combined_VOL, aes(x = CPu, y = Probe_d8,
  ...
  #geom_text(x=3,y=30,label=lm_eqn(geno_combined_VOL, 'Probe_d8','CPu'),color='red',parse=T) 
  ...
  labs(title = "Winding Number ~ Volume:CPu; males and females",
       #subtitle = "Both males and females",
       x = "CPu VOL", y = "WN: Probe Day d8") +
       theme_bw() +
...
plotCPuVOLSex <- ggplot(geno_combined_VOL, aes(CPu, Probe_d8, 
...
sink(paste(outpath, "Winding_Probed8_by_CPu_VOL.txt")) 
'CPu: Probe d8 AWN VOL'
lm <- lm(Probe_d8 ~ CPu*Genotype*Sex, geno_combined_VOL)
summary(lm)
lm$coefficients
'omnibus test'
...
'slopes'
print('VOL slopes')
em0<-emtrends(lm,  ~ Genotype, var = "CPu")
em1<-emtrends(lm, pairwise ~ Genotype, var = "CPu")
em2<-emtrends(lm, pairwise ~ Genotype|Sex, var = "CPu")
em3<-emtrends(lm, pairwise ~ Sex|Genotype, var = "CPu")

css0 <- summary(em0,infer=TRUE)
...
# Day d8 Probe By CPu FA

``` {r CPuFAybDayd8Probe}

plotCPuFA <- ggplot(geno_combined_FA, aes(x = CPu, y = Probe_d8,
                          color = Genotype,  fill = Genotype)) + 
  scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Sex)) +
  ...
  #             vstep = 0.05)+ # sets vertical spacing+
  theme_bw() +
  labs(title = "Winding Number ~ FA: CPu, males and females",
       #subtitle = "Both males and females",
       x = "CPu FA", y = "WN: Probe Day d8") +
       theme_bw() +
       theme(legend.position="bottom")+
       background_grid(major = "xy", minor = "none")
...
plotCPuFASex <- ggplot(geno_combined_FA, aes(CPu, Probe_d8, 
                                   color = Genotype, fill = Genotype))+                   scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Genotype)) + 
  facet_grid(. ~ Sex) + 
...
sink(paste(outpath, "Winding_Probed8_by_CPu_FA.txt")) 
'CPu: Probe d8 AWN FA'
lm <- lm(Probe_d8 ~ CPu*Genotype*Sex, geno_combined_FA)
'omnibus test'
joint_tests(ref_grid(lm)) 
...
# Day d8 Probe By CPu DEG

``` {r CPuDEGybDayd8Probe}

plotCPuDEG <- ggplot(geno_combined_DEG, aes(x = CPu, y = Probe_d8,
                          color = Genotype,  fill = Genotype)) + 
        scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
        scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
        geom_point(aes(shape = Sex)) +
       ...
       #        vstep = 0.05)+ # sets vertical spacing+
        theme_bw() +
        labs(title = "Winding Number ~ DEG: CPu, males and females",
       #subtitle = "Both males and females",
       x = "CPu DEG", y = "WN: Probe Day d8") +
       theme_bw() +
       theme(legend.position="bottom")+
       background_grid(major = "xy", minor = "none")
...
plotCPuDEGSex <- ggplot(geno_combined_DEG, aes(CPu, Probe_d8, 
                                   color = Genotype, fill = Genotype))+                   scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Genotype)) + 
  facet_grid(. ~ Sex) + 
...
sink(paste(outpath, "Winding_Probed8_by_CPu_DEG.txt")) 
'CPu: Probe d8 AWN DEG'
c
'omnibus test'
joint_tests(ref_grid(lm)) 
joint_tests(ref_grid(lm), by = "Sex", adjust = "sidak") 
...
# Day d8 Probe By CPu CLUS

``` {r CPuCLUSybDayd8Probe}




plotCPuCLUS <- ggplot(geno_combined_CLUS, aes(x = CPu, y = Probe_d8,
                          color = Genotype,  fill = Genotype)) + 
  scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Sex)) +
  ...
  labs(title = "Winding Number ~ CLUS: CPu, males and females",
       #subtitle = "Both males and females",
       x = "CPu CLUS", y = "WN: Probe Day d8") +
       theme_bw() +
       theme(legend.position="bottom")+
       background_grid(major = "xy", minor = "none")
...
plotCPuCLUSSex <- ggplot(geno_combined_CLUS, aes(CPu, Probe_d8, 
                                   color = Genotype, fill = Genotype))+                   scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Genotype)) + 
  facet_grid(. ~ Sex) + 
...
sink(paste(outpath, "Winding_Probed8_by_CPu_CLUS.txt")) 
'CPu: Probe d8 AWN CLUSTERING'
lm <- lm(Probe_d8 ~ CPu*Genotype*Sex, geno_combined_CLUS)
'omnibus test'
joint_tests(ref_grid(lm)) 
joint_tests(ref_grid(lm), by = "Sex", adjust = "sidak") 
...
pairs(lm.emm, by="Sex") 
'slopes'
#m.slopes <- lstrends(lm, 'Genotype', var='CPu')
#pairs(m.slopes)
...
``` {r CPuWindingbyMRIGraphs}

ggarrange(plotCPuVOL, plotCPuVOLSex, plotCPuFA, plotCPuFASex, 
          plotCPuDEG, plotCPuDEGSex, plotCPuCLUS, plotCPuCLUSSex,
          #labels = c("A: Day d8", "B: Day d8"),
          ncol = 2, nrow = 4)

ggsave(paste(outpath,'Probed8_Winding_by_CPu.pdf',sep=''), plot = last_plot(), device='pdf', scale=1, width=10, height=20, unit=c("in"), dpi=200)

---
title: "Absolute Winding Number by cbw Probe Day d5 "
...
# Day d5 Probe by cbw Vol


``` {r cbwVOLbbDayd5Probe, echo=FALSE}

plotcbwVOL <- ggplot(geno_combined_VOL, aes(x = cbw, y = Probe_d5,
  ...
  #geom_text(x=3,y=30,label=lm_eqn(geno_combined_VOL, 'Probe_d5','cbw'),color='red',parse=T) 
...
plotcbwVOLSex <- ggplot(geno_combined_VOL, aes(cbw, Probe_d5, 
...
sink(paste(outpath, "Winding_Probed5_by_cbw_VOL.txt")) 
'cbw: Probe d5 AWN VOL'
lm <- lm(Probe_d5 ~ cbw*Genotype*Sex, geno_combined_VOL)
summary(lm)
lm$coefficients
'omnibus test'
...
'slopes'
print('VOL slopes')
em1<-emtrends(lm, pairwise ~ Genotype, var = "cbw")
em2<-emtrends(lm, pairwise ~ Genotype|Sex, var = "cbw")
em3<-emtrends(lm, pairwise ~ Sex|Genotype, var = "cbw")
...
# Day d5 Probe By cbw FA

``` {r cbwFAybDayd5Probe}

plotcbwFA <- ggplot(geno_combined_FA, aes(x = cbw, y = Probe_d5,
                          color = Genotype,  fill = Genotype)) + 
  scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Sex)) +
...
plotcbwFASex <- ggplot(geno_combined_FA, aes(cbw, Probe_d5, 
                                   color = Genotype, fill = Genotype))+                   scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Genotype)) + 
  facet_grid(. ~ Sex) + 
...
sink(paste(outpath, "Winding_Probed5_by_cbw_FA.txt")) 
'cbw: Probe d5 AWN FA'
lm <- lm(Probe_d5 ~ cbw*Genotype*Sex, geno_combined_FA)
'omnibus test'
joint_tests(ref_grid(lm)) 
...
# Day d5 Probe By cbw DEG

``` {r cbwDEGybDayd5Probe}

plotcbwDEG <- ggplot(geno_combined_DEG, aes(x = cbw, y = Probe_d5,
                          color = Genotype,  fill = Genotype)) + 
        scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
        scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
        geom_point(aes(shape = Sex)) +
...
plotcbwDEGSex <- ggplot(geno_combined_DEG, aes(cbw, Probe_d5, 
                                   color = Genotype, fill = Genotype))+                   scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Genotype)) + 
  facet_grid(. ~ Sex) + 
...
sink(paste(outpath, "Winding_Probed5_by_cbw_DEG.txt")) 
'cbw: Probe d5 AWN DEG'
c
'omnibus test'
joint_tests(ref_grid(lm)) 
joint_tests(ref_grid(lm), by = "Sex", adjust = "sidak") 
...
# Day d5 Probe By cbw CLUS

``` {r cbwCLUSybDayd5Probe}




plotcbwCLUS <- ggplot(geno_combined_CLUS, aes(x = cbw, y = Probe_d5,
                          color = Genotype,  fill = Genotype)) + 
  scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Sex)) +
...
plotcbwCLUSSex <- ggplot(geno_combined_CLUS, aes(cbw, Probe_d5, 
                                   color = Genotype, fill = Genotype))+                   scale_color_manual(values=c('blueviolet', 'chartreuse1', 'red')) + 
  scale_fill_manual(values=c('blueviolet', 'chartreuse1', 'red'))+
  geom_point(aes(shape = Genotype)) + 
  facet_grid(. ~ Sex) + 
...
sink(paste(outpath, "Winding_Probed5_by_cbw_CLUS.txt")) 
'cbw: Probe d5 AWN CLUSTERING'
lm <- lm(Probe_d5 ~ cbw*Genotype*Sex, geno_combined_CLUS)
'omnibus test'
joint_tests(ref_grid(lm)) 
joint_tests(ref_grid(lm), by = "Sex", adjust = "sidak") 
...
ggsave(paste(outpath,'Probed5_Winding_by_cbw.pdf',sep=''), plot = last_plot(), device='pdf', scale=1, width=10, height=20, unit=c("in"), dpi=200)
